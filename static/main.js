...
function reloadall(gip,gport){
	...
	makeRequest(gip,gport)
		.then(({ status,nowplayer,maxplayer,motd,motd_json,version,timestamp,logo,ip,port }) => {
			...
			if(status===true){
				document.getElementById('status').innerHTML = '<span class="layui-badge layui-bg-green mu-2">在线</span>';
				...
				document.getElementById('status').innerHTML = '<span class="layui-badge layui-bg-red mu-2">离线</span>';
				...
				document.getElementById('motd').innerHTML = '<div class="layui-panel ly-motd-bg"><div style="padding:10px;color:red">获取服务器 MOTD 失败</div></div>';
